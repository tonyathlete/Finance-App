  ...
  return (
    <div className="w-full max-w-xl mx-auto mb-8">
      ...
      <div className="text-center mb-3 animate-fadeIn" key={step}>
        <p className="text-sm font-bold text-blue-700">{info.message}</p>
      </div>
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all duration-300 ${
                ...
                current ? 'bg-gradient-to-br from-blue-400 to-blue-500 text-white scale-110 shadow-md shadow-blue-200' :
                          'bg-blue-100 text-blue-400'
              }`}>
                {done ? '✓' : s.emoji}
              </div>
              <span className={`text-xs hidden md:block font-medium transition-colors ${current ? 'text-blue-700' : 'text-blue-300'}`}>
                {s.label}
              </span>
            </div>
          );
